...
DM = 20; %[pc cm^-3], dispersion measure
fc = 2000*10^6; %[Hz], centre frequency
sig_frb = I_data+1i*Q_data;
sig_original = real(sig_frb);                          % get real signal
f_start = 3000*1e6;   % start frequency of chirp
f_stop = 2500*1e6;     % end frequency of chirp
...
delays = zeros(1,M);
for i = 1:M/2
   delays(i) = 4.15*DM*(((2000/1000)^-2)-(((fc/1e6+frequencies(i))/1000)^-2))/1000;   %FRB delay
end
...
de_dispersed_data_plot = zeros(D,nof_clk);
...
for clk=1:nof_clk
    ...
    de_dispersed_data_plot(:,clk) = de_dispersed_data;
...
figure;
for i = 1:D
   plot(t_2*1e6,de_dispersed_data_plot(i,:));
   hold on;
end 
...
subplot(2,2,4);
imagesc(t_2*1e6,frequencies,de_dispersed_data_plot);
set(gca,'YDir','normal') ;
