...
A = 1;
X = [zeros(1,1024), A*chirp(t,20e6,0.000004,10e6)];
...
str_title8 = sprintf('Digital down conversion');
title(str_title8);
xlabel('Frequency/MHz'); ylabel('Magnitude');
...
%validVs1 = corr;
validVs1 = corr(Tx_hat+1:Tx_hat+length(refVs1));
%validVs1 = corr(length(refVs1):end-length(refVs1));


N = length(validVs1);
N_2 = ceil(N/2);
fax_Hz = (0:N-1)*(fs/N);
fftcorr = fft(validVs1);
subplot(3,1,3);
...
D = 16;
si = 1:length(h)/D;
sig_py = reshape(Vs1,D,length(Vs1)/D);
h_py = fliplr(reshape(h,D,length(h)/D));

sig_channel = zeros(D,length(Vs1)/D);
for i=1:D
    sig_channel(i,:) = downsample(filter(h_py(i,:),1,sig_py(i,:)),1);
end

sig_channel = fft(sig_channel);

figure;
for i=1:D
    subplot(4,4,i);
    plot(real(sig_channel(i,:)));
...
posVs1 = corr+abs(min(corr));                                 % move origin signal above zero    
% figure;
% plot(t,posVs1);
